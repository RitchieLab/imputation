...
if __name__ == "__main__":
	versMaj,versMin,versRev,versDate = 0,1,0,'2014-11-17'
	versStr = "%d.%d.%d (%s)" % (versMaj, versMin, versRev, versDate)
	versDesc = "impute2-segment-sort version %s" % versStr
	...
	parser = argparse.ArgumentParser(
		...
		epilog="""
example: %(prog)s -i shuffled_chr22 -o fixed_chr22

You may omit the --output option to simply test whether the input segments are
in order, without then sorting them.
"""
	)
	parser.add_argument('-i', '--input', type=str, metavar='prefix', required=True,
		help="prefix of impute2 *.best_guess_haps_imputation.* files to be segment-sorted (required)"
	)
	parser.add_argument('-o', '--output', type=str, nargs='?', metavar='prefix',
		help="prefix for sorted output files"
	)
	...
	if os.path.exists(args.input+'.best_guess_haps_imputation.impute2.gz'):
		genoFile = zopen(args.input+'.best_guess_haps_imputation.impute2.gz')
	elif os.path.exists(args.input+'.best_guess_haps_imputation.impute2'):
		genoFile = open(args.input+'.best_guess_haps_imputation.impute2','rU')
	else:
		exit("ERROR: %s.best_guess_haps_imputation.impute2(.gz) not found" % args.input)
	if os.path.exists(args.input+'.best_guess_haps_imputation.impute2_info.gz'):
		infoFile = zopen(args.input+'.best_guess_haps_imputation.impute2_info.gz')
	elif os.path.exists(args.input+'.best_guess_haps_imputation.impute2_info'):
		infoFile = open(args.input+'.best_guess_haps_imputation.impute2_info','rU')
	else:
		exit("ERROR: %s.best_guess_haps_imputation.impute2_info(.gz) not found" % args.input)
	print "... OK"
	
	# scan and split input files
	size = 0
	header = "snp_id rs_id position exp_freq_a1 info certainty type info_type0 concord_type0 r2_type0"
	for line in itertools.chain(infoFile, [header]):
		if line == header:
			if size:
